...
//    mtReserved0,	  //  0	Reserved
  ...
  TMQTTMsgEvent = procedure (Sender : TObject; aTopic : UTF8String; aMessage : string; aQos : TMQTTQOSType; aRetained : boolean) of object;
  TMQTTRetainEvent = procedure (Sender : TObject; aTopic : UTF8String; aMessage : RawByteString; aQos : TMQTTQOSType) of object;
  TMQTTRetainedEvent = procedure (Sender : TObject; Subscribed : UTF8String; var aTopic : UTF8String; var aMessage : RawByteString; var aQos : TMQTTQOSType) of object;
  ...
  public
    ...
    procedure Parse (aStr : RawByteString); overload;
    ...
    procedure SendPublish (anID : Word; aTopic: UTF8String; aMessage : string ; aQOS : TMQTTQOSType; aDup : boolean = false; aRetain : boolean = false);
    procedure SendPing;
    procedure SendDisconnect;
    procedure SendSubscribe (anID : Word; aTopic : string; aQOS : TMQTTQOSType); overload;
...
begin
  ...
  {$IFDEF NEXTGEN}
      aStream.Write (aStr[0], length (aStr));
  {$ELSE}
...
procedure TMQTTParser.Parse (aStr: RawByteString);
var
                  ...
                  mtPUBLISH  :
                    if FRxStream.Size >= 4 then
                      begin
                        ...
                        len := FRxStream.Size - FRxStream.Position;
                        if len > 0 then
                        begin
                          SetLength(bytes, len);
                          FRxStream.Read(bytes, len);
//                          coding := TEncoding.GetEncoding(936);
//                          try
//                            Str := coding.GetString(bytes);
//                          finally
//                            coding.Free;
//                          end;
                          try
...
procedure TMQTTParser.SendPublish (anID : Word; aTopic : UTF8String; aMessage : string; aQos : TMQTTQOSType; aDup : boolean = false; aRetain : boolean = false);
var
  s : TMemoryStream;
  bytes: TBytes;
  coding: TEncoding;
...
//      coding := TEncoding.GetEncoding(936);
//      try
//        bytes := coding.GetBytes(aMessage);
//      finally
//        coding.Free;
//      end;
...
procedure TMQTTParser.SendSubscribe (anID : Word; aTopic : string; aQOS : TMQTTQOSType);
begin
